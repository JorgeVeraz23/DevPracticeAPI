...
namespace PuntoDeVentaAPI.Controllers.DevPracticeController.CarritoDeComprasController
{
    ...
    public class UsuarioController : ControllerBase
    {
        ...
        private static Logger _log = LogManager.GetLogger("UsuarioController");
        MessageInfoDTO infoDTO = new MessageInfoDTO();
        public readonly string _usuario;
        private readonly string _ip;
        private readonly string _nombreController;

        public UsuarioController(UsuarioInterface usuarioInterface, ApplicationDbContext applicationDbContext, IHttpContextAccessor httpContextAccessor, ApplicationUserManager userManager, IServiceProvider service, IMapper mapper, IConfiguration configuration)
        {
            ...
            _nombreController = "UsuarioController";
        ...
        public async Task<ActionResult> GetAllUsuarios()
        {
            try
            {
                var result = await _usuarioInterface.GetAll();
                return Ok(result);
            }catch (Exception ex)
            {
